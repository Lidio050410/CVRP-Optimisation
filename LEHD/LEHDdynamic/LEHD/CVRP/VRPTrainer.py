...
class VRPTrainer:
    ...
    def _train_one_batch(self, episode,batch_size,epoch):
        ...
        # 计算实际评分（示例：使用学生解的路径长度）
        problems = self.env.problems[:, :, [0, 1]]  # 坐标部分
        student_length = self.env.cal_length(
            problems,
            self.env.selected_student_list,
            self.env.selected_student_flag
        )
        avg_score = student_length.mean().item()
    
        return avg_score, avg_score, loss_mean.item()  # 返回实际评分
