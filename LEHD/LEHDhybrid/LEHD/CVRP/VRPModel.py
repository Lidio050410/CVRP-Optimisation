...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        ...
        self.add_q_norm = AddAndInstanceNormalization(**model_params)
        self.add_k_norm = AddAndInstanceNormalization(**model_params)
        self.add_v_norm = AddAndInstanceNormalization(**model_params)
    ...
    def forward(self, input1):
        ...
        # 调整 q/k/v 形状为 (batch, problem, head_num * qkv_dim) 以匹配 input1
        q_reshaped = q.transpose(1, 2).reshape(q.size(0), q.size(2), -1)  # (batch, problem, head_num*qkv_dim)
        k_reshaped = k.transpose(1, 2).reshape(k.size(0), k.size(2), -1)
        v_reshaped = v.transpose(1, 2).reshape(v.size(0), v.size(2), -1)

        # 对调整后的 q/k/v 进行归一化
        q_normalized = self.add_q_norm(input1, q_reshaped)  # input1: (batch, problem, embedding_dim)
        k_normalized = self.add_k_norm(input1, k_reshaped)
        v_normalized = self.add_v_norm(input1, v_reshaped)

        # 恢复 q/k/v 的多头形状
        q = q_normalized.reshape(q.size(0), q.size(2), head_num, qkv_dim).transpose(1, 2)
        k = k_normalized.reshape(k.size(0), k.size(2), head_num, qkv_dim).transpose(1, 2)
        v = v_normalized.reshape(v.size(0), v.size(2), head_num, qkv_dim).transpose(1, 2)
